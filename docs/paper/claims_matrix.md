...
| C7 | Diffusion-based generative surprise `Sgen` separates anomalies | `cut_unless_fixed` | AGAINST: `artifacts/day34/day34_generative_eval_summary.json` — Sgen ROC-AUC 0.475 (worse than random); anomaly mean < normal mean | Phase 5 decision gate (multi-sample Sgen). Cut from main claim unless gate passes |
| C8 | Diffusion model generates realistic EHR sequences | `cut_unless_fixed` | AGAINST: generated length ~254 vs real ~47; 127 of 4,587 tokens used (mode collapse), JS 0.27 | Phase 5: discrete/latent diffusion or cut |
| C9 | Diffusion-based counterfactual generation | `cut_unless_fixed` | AGAINST: `src/explanations/counterfactual.py` performs a rule-based edit search; the diffusion model is never invoked in counterfactual generation | Phase 4 (and only if Phase 5 passes for a generative variant) |
| C10 | Ontology-guided counterfactual repair (minimal, plausible edits) | `cut_unless_fixed` | AGAINST: edits are drawn from injection answer-key columns (`bad_code`, `expected_code`, `replacement_code`) and scored by the same rule set — circular | Phase 4: leakage-free, ontology-neighborhood edits, independent scorer |
...
- C7 Sgen separation, C8 realistic generation,
  C9 diffusion counterfactual, C10 ontology counterfactual repair, C12 A* novelty framing.
- (C2b non-circular benchmark **resolved to `supported_now` (scoped)** in Phase 1b. C5 real ontology integration **resolved to `supported_now` (scoped)** in Phase 2b-fix. C4 ontology ranking **upgraded to `partially_supported`** in Phase 3b — real standalone signal 0.79.)
...
- (Conditionally) generative surprise and diffusion repair, **if** the Phase 5 gate fails.
...
3. "Your counterfactual reads the corruption note / answer-key columns." → Phase 4 (open).
4. "Your generative surprise is worse than random; why keep it?" → Phase 5 (open); `w_gen=0` for now.
